...
public abstract class SteeringAgentSoftmaxActionSoftmaxPolicySelection implements AgentInterface {
    ...
    double lambda = 0.95;
    double startingTemperature = 2;//***for softmax selection
    double temperature;
    ...
    public Action agent_step(Reward reward, Observation observation) 
    {
        ...
        if (!policyFrozen) {
            for (int i = 0; i < numPolicies; i++) {
                ...
                for (int j = tracingStack.size() - 1; j >= 0; j--) 
                {
                    ...
                    //softmaxAction = getSoftmaxAction(newState, i);
                    ...
                    if (j + 1 == tracingStack.size()) {
                        ...
                        //valueFunction.calculateErrors( prevAction, prevState, softmaxAction, newState, gamma, reward );
        ...
        //***
        //int action = getSoftmaxAction(newState, bestPolicy);
   ...
   //*** 
    private int getSoftmaxAction(int state, int policy)
    {
        ...
        int selectedAction = Softmax.getAction(actionValues, temperature, greedyAction);
...
//        System.out.println("number of actions " + numActions);
    ...
    protected void agent_start_internal_episodic(int state)
    {
    	for (int i=0; i<numPolicies; i++)
    	{
    		...
    		//***
    		int action = getSoftmaxAction(state, i);
            ValueFunction valueFunction = valueFunctions.get(i);
    		for (int j=0; j<numObjectives; j++)
    		{
...
//            bestPolicy =  bestPolicies.get(0);
        ...
        bestPolicy = Softmax.getAction(policyValues, temperature, greedyPolicy);
