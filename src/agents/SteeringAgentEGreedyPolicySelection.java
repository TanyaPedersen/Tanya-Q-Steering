...
public abstract class SteeringAgentEGreedyPolicySelection implements AgentInterface {
    ...
    double alpha = 0.9;
    double startingEpsilon = 0.9; // use .9 for DST and .2 for LinkedRings
    ...
    double epsilon;
    double startingTemperature = 15;//***for softmax selection
    double temperature;
    double temperatureDecay = startingTemperature/5000;
    double gamma = 1.0; // use 1.0 for DST, 0.9 for LinkedRings
    double lambda = 0.95;
    int numObjectives;
    ...
    public Action agent_start(Observation observation) 
    {
    	...
    	if (numEpisodes==0)
    	{
    		...
    		softmaxPolicy = chooseSoftmaxPolicy_Q(targetVector, state);
    		
    		greedyPolicy = chooseEGreedyPolicy_Q(targetVector, state);
    	}
    	else
    	{
        ...
        //***
    	//int action = getSoftmaxAction(state, softmaxPolicy); 
    	int action = getAction(state, greedyPolicy);  
    ...
    public Action agent_step(Reward reward, Observation observation) 
    {
    	...
    	//***
    	//System.out.println("num episodes " + numEpisodes);
    	//System.out.println("num steps " + numOfSteps);
        ...
        //***
        //int action = getSoftmaxAction(newState, softmaxPolicy);
        int action = getAction(newState, greedyPolicy);
        Action returnAction = new Action(1, 0, 0);
        returnAction.intArray[0] = action;
        StateActionDiscrete sa = new StateActionDiscrete(observation, returnAction);
  ...
  //*** 
    private int getSoftmaxAction(int state, int policy)
    {

    	
    	int greedyAction = getGreedyAction(state, policy);
    	int action;
    	ValueFunction valueFunction = valueFunctions.get(policy);;
		...
		for(int i =0; i<numActions; i++)
		{
			action = i;
			dotProduct = 0.0;
			double [] futureRewards = valueFunction.getQValues( action , state );
	        ...
	        actionValues[i] = dotProduct;
		}	
		int selectedAction;		
        
		if (!policyFrozen) {
            if (random.nextDouble() < epsilon) {
                selectedAction = random.nextInt(numActions);
            } else {
                selectedAction = Softmax.getAction(actionValues, temperature, greedyAction);
            }
        } 
        else 
        {
    ...
    protected void agent_step_internal_non_episodic(Reward reward, int newState)
    {
    	...
    	//***
    	//softmaxPolicy = chooseSoftmaxPolicy_Q(targetVector, newState);
    	greedyPolicy = chooseEGreedyPolicy_Q(targetVector,newState);
    ...
    //***
    protected int chooseSoftmaxPolicy_Q(double[] targetVector, int state) {

        
        
        double[] policyValues = new double[numPolicies]; 
        ...
        for (int i = 0; i < numPolicies; i++) {
        	...
        	ValueFunction valueFunction = valueFunctions.get(i);            
            //***
            int selectedAction = getSoftmaxAction(state, i);
            ...
            policyValues[i] = dotProduct;
                    

        }
        //System.out.print(bestPolicy);
        //***
        int greedyPolicy = chooseGreedyPolicy_Q(targetVector, state);
        softmaxPolicy = Softmax.getAction(policyValues, temperature, greedyPolicy);
        
        return softmaxPolicy;
    }
    //***
    protected int chooseEGreedyPolicy_Q(double[] targetVector, int state) {
        ...
        if (random.nextDouble()<=epsilon)
    		...
    		bestPolicy = chooseGreedyPolicy_Q(targetVector, state);
